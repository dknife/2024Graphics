...
class MyCam :
    def __init__(self):
        self.loc = np.array([0.0, 1.5, 0.0]) # 카메라 위치
...
def drawPlane_prepareBuffer() :
    ...
    vBuffer = [] # 정점 정보를 담은 버퍼
    cBuffer = [] # 각 정점의 색상을 담은 버퍼
    ...
    for i in range(n):
        for j in range(n):
            if (i+j)%2 == 0 :
                X = startX + i*d
                Z = startZ + j*d
                vBuffer.append([X, 0.3, Z])
                vBuffer.append([X+d, 0.3, Z])
                vBuffer.append([X+d, 0.3, Z+d])
                vBuffer.append([X, 0.3, Z+d])   
                cBuffer.append([1, 0, 0])
                cBuffer.append([0, 1, 0])
                cBuffer.append([0, 0, 1])
                cBuffer.append([1, 1, 0])
...
class MyGLWidget(QOpenGLWidget):
    ...
    def initializeGL(self) :
        ...
        self.myVertexArray, self.myColorArray = drawPlane_prepareBuffer() 
        ...
        glColorPointer(3, GL_FLOAT, 0, self.myColorArray)
    ...
    def paintGL(self):        
        ...
        # 버텍스 버퍼 그리기
        glColor3f(0, 0, 1)        
        glDrawArrays(GL_QUADS, 0, len(self.myColorArray))
